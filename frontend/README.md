...
Angular 15 frontend for the POS system, based on Nebular/ngx-admin.

## Production deployment

Build the optimized application with:
...
Serve the generated `dist/` directory with SPA fallback to `index.html`. The included `Dockerfile` builds the application and serves it with Nginx using the included `nginx.conf`.

Production defaults to the same-origin API path `/v1`. For a separately hosted API, set `window.__POS_CONFIG__.apiBaseUrl` in `assets/runtime-config.js` at deployment time. The Stripe browser key can be set as `window.__POS_CONFIG__.stripeKey`; never place a Stripe secret key in the frontend.

The GitHub Actions workflow runs the production build on every push and pull request.
